...
my_flights %>% 
  ...
  group_by(carrier) %>% 
  summarize(on_time_rate = mean(on_time)) %>% 
    left_join(airlines, join_by(carrier)) %>% 
...
## Yes, they are returning the same rows because every carrier exists in the table.
```
...
## These are flights that are not in the faa table.

anti_join(airports, flights, by = c("faa" = "dest"))

## Airports without flights there.
```
